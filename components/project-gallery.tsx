...
import { Github, Play, ExternalLink, ChevronLeft, ChevronRight } from "lucide-react"
...
function YearRow({ year, projects, onProjectClick }: YearRowProps) {
            ...
            <div className="relative group">
                ...
                >
                    {projects.map((project) => (
                        ...
                        >
                            <div className="mb-4 overflow-hidden rounded-t-lg bg-muted/10 aspect-video relative group-hover/card:border-primary/20 transition-colors duration-500">
                                <img
                                    src={project.image}
                                    alt={project.title}
                            ...
                            <div className="p-5 flex-1 flex flex-col">
                                <div className="flex flex-wrap gap-1.5 mb-2.5">
                                    {project.tags.map((tag) => (
                                        <span
                                            key={tag}
                                            className="px-2 py-0.5 font-mono text-[9px] tracking-wider rounded-full bg-primary/5 text-primary/70 border border-primary/10"
                                        >
                                            {tag}
                                        </span>
                                    ))}
                                ...
                                <h3 className="font-mono text-sm text-foreground dark:text-[#BDE0FE] mb-1.5 leading-snug group-hover/card:text-primary transition-colors font-bold">
                                    {project.title}
                                </h3>
                                <p className="text-xs text-muted-foreground dark:text-[#BDE0FE]/60 leading-relaxed line-clamp-2 mb-4 flex-1">
                                    {project.description}
                                </p>
                                ...
                                <div className="flex items-center justify-between pt-3 border-t border-border/40 mt-auto">
                                    <div className="flex items-center gap-3">
                                        ...
                                        {project.demoVideo && <Play className="w-3.5 h-3.5 text-primary/60" />}
                                    </div>
